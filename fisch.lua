-- ERRORS WILL BE HREE < EXPECT NOTHING
...
local Window = Library:CreateWindow({
    Title = 'Fisch | Mimialist hub (not many features)',
    Center = true,
    AutoShow = true,
    TabPadding = 8,
    MenuFadeTime = 0.2
...
local WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('Eclipse Hub V1.2.3 | %s fps | %s ms'):format(
        math.floor(FPS),
...
local AutoShakeGroup = Tabs.Main:AddLeftGroupbox('auto shaek')
local AutoShakeSettings = Tabs.Main:AddLeftGroupbox('Equip rod first')
local AutoReelGroup = Tabs.Main:AddLeftGroupbox('avtoreel')
...
AutoShakeKeyCodeEventText:AddLabel('Inspired from noone')
AutoShakeKeyCodeEventText:AddLabel('EQUIP ROD FIRST')
...
CreditsGroup:AddLabel('Made Eclipse')
CreditsGroup:AddLabel('yes')

CreditsGroup:AddLabel('ohio')
...
task.spawn(function()
    local success, LatestVer = pcall(function()
        local version = game:HttpGet("skibidi")
